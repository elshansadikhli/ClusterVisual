  ...
  const _gene = {
    ...
    labelTransform: (g) => {
      ...
      let rotate = ["start", "middle"].includes(config.gene.label.anchor)
        ...
        : config.gene.label.rotation;
      return `translate(${gx - 5}, ${gy - 8}) rotate(${rotate})`; //Modified to show label higher
    },
    labelDy: () => {
    ...
    tooltipHTML: (g) => {
      ...
      text.on("input", (e) => {
        g.name = e.target.value; //Modified to edit g.name instead of g.label
        select.attr("value", null);
        plot.update({});
      });
      select.on("change", (e) => {
        g.name = e.target.value;
        text.attr("value", e.target.value); //Modified to edit g.name instead of g.label
        plot.update({});
      });
      return div;
    },
    ...
    labelText: (g) => g.name || g.uid, //Modified to show a gene name insted of g.label
    polygonClass: (g) => {
  ...
  const _group = {
    tooltipHTML: (g) => {
      ...
      let text = div
        .append("input")
        .attr("type", "input")
        .attr("value", g.name || g.uid); //Modified to show g.name instead of g.label
      ...
      // Add event handlers to update labels
      text.on("input", (e) => {
        g.name = e.target.value; //Modified to edit the g.name instead of label
        select.attr("value", null);
        plot.update({});
      });
      return div;
    ...
    contextMenu: (event, data) => {
      ...
      // Modified not to be shown on the chart
      // Get position relative to clicked element
      let rect = event.target.getBoundingClientRect();
      let bbox = tip.node().getBoundingClientRect();
        ...
        .join(
          (enter) => {
            ...
            enter
              ...
              .style("width", "20%")
              .style("height", "40%")
              .style("background-color", "rgba(128, 128, 128, 0.151)");
            ...
            // Modidied to be shown near the plot
